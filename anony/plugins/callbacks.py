    ...
    if action == "pause":
        ...
        if not media or pos == -1:
            return await app.edit_message_text(
                ...
                rich_message=rich(query.lang["play_expired"]),
            )
        ...
        msg = await app.send_rich_message(
            chat_id=chat_id,
            rich_message=rich(query.lang["play_next"]),
        )
    ...
    try:
        if action in ["skip", "replay", "stop"]:
            await app.send_rich_message(
                chat_id=query.message.chat.id,
                rich_message=rich(reply),
                reply_parameters=types.ReplyParameters(
                    message_id=query.message.reply_to_message_id
                )
                if query.message.reply_to_message_id
                else None,
            )
            await query.message.delete()
        else:
            mtext = (
                query.message.caption.markdown
                if query.message.caption
                else query.message.text.markdown
            )
...
async def _help(_, query: types.CallbackQuery):
    ...
    if data[1] == "back":
        return await app.edit_message_text(
            chat_id=query.message.chat.id,
            message_id=query.message.id,
            rich_message=rich(query.lang["help_menu"]),
    ...
    return await app.edit_message_text(
        chat_id=query.message.chat.id,
        message_id=query.message.id,
        rich_message=rich(query.lang[f"help_{data[1]}"]),
        reply_markup=buttons.help_markup(
            query.lang,
            back=True,
        ),
