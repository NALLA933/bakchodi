...
class Utilities:
    ...
    async def play_log(
        ...
        await app.send_rich_message(chat_id=app.logger, rich_message=rich(_text))
    ...
    async def send_log(self, m: types.Message, chat: bool = False) -> None:
        if chat:
            ...
            return await app.send_rich_message(
                chat_id=app.logger,
                rich_message=rich(
                    m.lang["log_chat"].format(
                        m.chat.id,
                        m.chat.title,
                        user.id if user else 0,
                        user.first_name if user else "Anonymous",
                    )
                ),
            )
