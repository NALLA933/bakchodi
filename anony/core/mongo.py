...
class MongoDB:
    ...
    async def get_assistant(self, chat_id: int):
        ...
        if not (1 <= num <= len(anon.clients)):
            logger.warning(f"Assistant index {num} out of range for chat {chat_id}, reassigning")
    ...
    async def get_client(self, chat_id: int):
        ...
        if client is None:
            logger.warning(f"No named client for assistant index {num}, chat {chat_id}, reassigning")
            num = await self.set_assistant(chat_id)
            client = clients.get(num)
        return client
